...
An \textbf{agentic LLM architecture} is introduced that decomposes the dual
...
the workflow separates \textit{generation}, \textit{critique},
\textit{refinement}, and \textit{verification} into distinct computational
nodes with typed interfaces. Figure~\ref{fig:arch} provides an overview.
...
Marker extraction is formulated as a \textbf{hybrid extraction strategy} that
...
token counting \cite{fu2024struggle}. Accordingly, a two-stage strategy is
used: (i) the LLM serves as a \textbf{semantic reasoner}, identifying marker
text strings and their labels through reasoning chains; (ii) a deterministic
Python-based \textbf{structural locator} performs exact string matching to
compute character offsets (\texttt{startIndex}, \texttt{endIndex}). This hybrid
approach eliminates the ``hallucinated span'' problem, i.e. instances where the
LLM correctly identifies marker semantics but produces invalid or off-by-one
...
\paragraph{Dynamic Discriminative Chain-of-Thought (DD-CoT).} The core methodological contribution extends standard chain-of-thought (CoT)
...
identified during early development: label confusion between semantically
...
\paragraph{Agents} A Self-Refine loop is implemented with four nodes: (i)~a DD-CoT
\textbf{Generator} proposes labeled marker strings; (ii)~an \textbf{Enhanced
    Critic} audits verbatimness, boundaries, labels, and missed markers; (iii)~a
\textbf{Refiner} applies targeted fixes; (iv)~a \textbf{Deterministic Verifier}
anchors marker strings to exact character offsets and deduplicates overlaps.
The verifier's multi-tier matching cascade is detailed in
...
The principal challenge in conspiracy detection is distinguishing \textit{topic
    presence} from \textit{stance endorsement}, a phenomenon termed the
...
reason, S2 is implemented as a three-node graph maintaining: document text,
markers extracted from S1, marker summary, few-shot retrieval context, forensic
statistics, council votes, and final verdict. The pipeline executes
sequentially: Forensic Profiler (deterministic metrics) $\rightarrow$ Parallel
Council $\rightarrow$ Calibrated Judge.
...
``JAQing'' patterns) that are injected as structured warnings. Full metric
definitions are moved to Appendix~\ref{app:forensic}.
...
\paragraph{Parallel Council Architecture.} To force genuine epistemic diversity, an \textbf{Anti-Echo Chamber} where four
specialized personas vote \textit{independently} without information leakage:
\begin{enumerate}
...
All four agents execute independently within a single graph node, where each
juror receives identical inputs and produces its vote without access to other
jurors' outputs, ensuring epistemic independence and preventing ordering bias
observed in sequential debate architectures. In the current implementation, API
calls are serialized with rate-limit management, though the architecture
supports true parallel execution.%\textit{steelman} of the opposing view 
...
The official SemEval train/dev splits are used without modification. All agent
nodes use GPT-5.2 with schema-constrained generation via PydanticAI
\cite{pydanticai2024} and are orchestrated via LangGraph \cite{langgraph2024}.
Retrieval is implemented over a vector store with reranking; full preprocessing
and optimization details (including MMR and GEPA) are moved to
...
Table~\ref{tab:main_results} reports macro F1 for the agentic system versus a
zero-shot GPT-5.2 baseline on dev and the official test set.
...
\textbf{S1: Marker Extraction} improves substantially on dev (0.12 $\rightarrow$
0.24); ablations are summarized in Section~\ref{sec:ablation}.

\textbf{S2: Conspiracy Detection} improves on dev (0.53 $\rightarrow$ 0.79);
ablations are summarized in Section~\ref{sec:ablation}.
...
\begin{table}[t]
    ...
    \begin{tabular}{@{}p{3.05cm}p{1.85cm}rrr@{}}
        ...
        \midrule
        \textbf{S1 Self-Refine}                                                                                \\(Gen $\rightarrow$ +Critic+Refiner) & Macro F1 (dev) & 0.173 & 0.240 & +0.067 \\
        ...
        \bottomrule
    \end{tabular}
    \caption{Combined ablation summary (dev). ``Base''/``New'' correspond to the paired settings reported in the original ablation tables; full tables and narratives are moved to Appendix~\ref{app:ablation-full}.}
    \label{tab:combined_ablation}
\end{table}

\paragraph{Notable findings.}
On S1, the Self-Refine loop accounts for the largest gain (+0.067 macro F1 on
dev), while DD-CoT improves agency identification (+2.7 Actor F1). On S2, the
Parallel Council increases recall (0.481 $\rightarrow$ 0.560), and contrastive
few-shot retrieval halves the false-positive rate (0.160 $\rightarrow$ 0.080).
Residual errors are largely driven by boundary drift/role confusion (S1) and
implicit stance or sarcasm (S2) (Appendix~\ref{app:qualitative}).

Additional ablation narratives, retrieval analysis, and the full qualitative
error analysis are moved to Appendix~\ref{app:ablation-full} and
Appendix~\ref{app:qualitative}.

\section{Conclusion}
This paper presented an agentic LLM system for SemEval-2026 Task 10 that
improves both marker extraction (S1) and conspiracy detection (S2) through
workflow-level decomposition and targeted inductive biases. For S1, the system
combines DD-CoT discriminative reasoning with a hybrid extraction strategy that
separates semantic identification from deterministic span localization,
yielding structurally valid character offsets. For S2, an Anti-Echo Chamber
integrates lightweight forensic profiling with a parallel council and a
calibrated judge to reduce false positives on reporting/debunking content (the
Reporter Trap). In addition, contrastive few-shot retrieval supplies
discriminative in-context precedents without augmentation. Remaining errors
stem from pragmatic ambiguity and limited discourse context.
...
Rate by 50\% (0.160 $\rightarrow$ 0.080), validating that retrieval strategy
...
$\rightarrow$ \texttt{\textless extraction\_ontology\textgreater} $\rightarrow$
